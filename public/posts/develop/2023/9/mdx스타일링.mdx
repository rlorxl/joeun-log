...
이렇게 구현은 했는데 ref를 전달하는것과 각 태그마다 useScroll이 호출되는것이 찜찜하다.

forwardRef로 DOM컨텐츠를 전달하는 형태는 많이 봤는데 ref값 자체를 전달해도 되나 하는 의문이다.

일단 스크롤이벤트에는 너무 많은 호출이 일어나지 않도록 쓰로틀링을 적용하고 스크롤 함수는 useCallback을 적용했다.

아직까지는 이게 올바른 코드인지 판단이 잘 안간다. 구현은 됐는데...

코드리뷰를 하고 이에대한 고민을 나눌 수 있는 동료가 있으면 좋겠다.
