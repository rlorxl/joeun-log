...
그러다가 그냥 뇌피셜로 왠지 SSG방식을 적용해볼까 하는 생각이 들었고 generateStaticParams를 적용해보려고 끙끙댔다..
당연히 그 문제가 아니라서 해결되지 않았고 애꿎은 generateStaticParams를 붙잡고 씨름했다...
파일을 찾아올 때 uri디코딩이 문제인가? 싶어서 시도해봤는데 그 문제도 아니었고 이렇게 삽질하는 과정에서 대략 5시간쯤 날린거 같다(^^)

그러다가 뒤늦게 서버로그를 확인해 보라는 문구를 보고 로그를 확인해보면서 오류가 발생하는 폴더가
캐치올 폴더가 아닌 다이나믹 라우트 파일안이라는 사실을 알게됐다.
...
공식문서를 찬찬히 다시 봤는데 뭐 같이 쓰지 말라는 내용은 딱히 없었고 (당연한건가?ㅠㅠ) 캐치올 라우트가 `/blog/a`부터 연결되니까 `/blog/a`경로로 들어갔을때 캐치올 라우트의 파일이 연결될 순 있을것 같은데
`/blog/develop/a/b/c`경로로 연결했는데 왜 다이나믹 라우트로 연결되는걸까...????????
그리고 개발모드로 작업할 때는 문제가 없었다.
어쨋든 오류가 다이나믹 라우트 폴더의 페이지에서 생기고 있으니 테스트로 다이나믹 라우트를 삭제했고 캐치올라우트 파일 안에서 segment의 길이를 통해 어떤 경로인지 판별하고
각기 다르게 데이터를 불러오고 다른 컴포넌트를 렌더링하도록 수정했더니 게시물 페이지로 잘 이동이 됐다.
...
이 문제랑 다른 오류가 하나 더 있어서 꼬박 하루가걸려 70커밋을 하고 난 후에야 오류 두개를 잡을 수 있었다.🥲
오랜만에 오래걸린 오류였고 예상하지 못한곳에서 발생해서 해결하는데 더 머리가 아팠던거 같다. 한가지 내가 잘못한 것은 중간에 너무 나의 뇌피셜로 해결을 하려고 했다는것이다.
...
덕분에 generateStaticParams로 SSG를 구현하는 것까지 공부했으니까 오히려 좋다고 스스로 위로를 해본다 ~🤧
