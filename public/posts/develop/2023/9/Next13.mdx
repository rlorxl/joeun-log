...
가장 생소하게 느껴졌던 부분. 컴포넌트는 서버컴포넌트와 클라이언트 컴포넌트로 나뉘어진다.
...
이처럼 서버 컴포넌트와 클라이언트 컴포넌트의 역할이 다르고 각 컴포넌트는 지정된 규칙에 따라서 렌더링 되어지기 때문에 자칫 넋놓고 코딩하다가는 쉽게 에러를 마주하게 된다. (엉엉)
...
Next.js는 네이티브 fetch Web API를 제공한다.
서버에서의 각 fetch요청에는 캐싱과 재요청을 할 수 있고 컴포넌트 트리가 렌더링되는 동안 자동적으로 메모이즈 된다.
